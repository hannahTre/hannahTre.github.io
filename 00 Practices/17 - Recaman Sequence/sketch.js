...
// Hannah Tremaine
// 11/14/2023
// OOP pratice, interesting number sequence, lerp(),dynamic scaling

let sequence = []; //array to hold recman numbers
let stepAmount = 1;
let currentValue = 0;
let largest = 0;
let scaleAmount = 0;
...
function addToSequence(){
  //generate the next number in the recman sequence
  let backwards = currentValue - stepAmount;
  if(backwards>0 && !sequence.includes(backwards)){
    //do some drawing stuff here later
...
function draw() {
  ...
  addToSequence();
  scaleAmount = lerp(scaleAmount,width/largest,0.005);
  scale(scaleAmount);
  renderArcs();
}
...
class rArc{
  constructor(start,end,direction){
    this.start = start;
    this.end = end;
    this.direction = direction; //0-forward:upper arc 1-backward:lower arc
  }
  display(){
